...
fsync_decoder_t *fsync_decoder_new(int sampleRate) {
  ...
  if (!decoder) return (fsync_decoder_t *)0L;
  ...
  for (i = 0; i < FSYNC_ND; i++) {
    decoder->th[i] =
        0.0 + (((fsync_float_t)i) * (TWOPI / (fsync_float_t)FSYNC_ND_12));
    while (decoder->th[i] >= TWOPI) decoder->th[i] -= TWOPI;
...
static void _dispatch(fsync_decoder_t *decoder, int x) {
  ...
  if (fleetflag) {
    if (msglen < 7) return;  // cannot dispatch, not enough data
    to_fleet = msg[6] + 99;
  } else {
    to_fleet = from_fleet;
  }
  ...
  // printf("From:%d %d\n", from_fleet,from_unit);
  // printf("To:%d %d\n", to_fleet,to_unit);

  if (m1 == 0x42) {
    int i, offset;

    if (msglen < 11) return;  // truncated before length field, cannot dispatch
    ...
    for (i = 0; i < paylen; i++) {
      ...
      if (offset >= msglen)
        return;  // truncated message (XXX could instead dispatch what we have
                 // but with a truncated flag
      if (i >= sizeof(payload))
  ...
  if (decoder->callback)
    (decoder->callback)((int)m1, (int)m0, (int)from_fleet, (int)from_unit,
                        (int)to_fleet, (int)to_unit, (int)aflag, payload,
                        paylen, (unsigned char *)msg, (int)msglen,
                        decoder->callback_context, decoder->is_fs2[x],
                        x < FSYNC_ND_12 ? 0 : 1);
  // printf("We have a callback i guess\n");
}
...
static void _procbits(fsync_decoder_t *decoder, int x) {
  ...
  {
    // printf("valid CRC\n");
    decoder->message[x][decoder->msglen[x]++] =
        (decoder->word1[x] >> 24) & 0xff;
    decoder->message[x][decoder->msglen[x]++] =
        (decoder->word1[x] >> 16) & 0xff;
    decoder->message[x][decoder->msglen[x]++] = (decoder->word1[x] >> 8) & 0xff;
    decoder->message[x][decoder->msglen[x]++] = (decoder->word1[x] >> 0) & 0xff;
    decoder->message[x][decoder->msglen[x]++] =
        (decoder->word2[x] >> 24) & 0xff;
    decoder->message[x][decoder->msglen[x]++] =
        (decoder->word2[x] >> 16) & 0xff;
    ...
    if (ec1 != -1 && ec2 != -1 && ec3 != -1 && ec4 != -4) {
      switch (decoder->fs2state[x]) {
        ...
        case 3:
          ...
          {
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w1[x] >> 24) & 0xff;
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w1[x] >> 16) & 0xff;
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w1[x] >> 8) & 0xff;
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w1[x] >> 0) & 0xff;
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w2[x] >> 24) & 0xff;
            decoder->message[x][decoder->msglen[x]++] =
                (decoder->fs2w2[x] >> 16) & 0xff;
      ...
      if (decoder->msglen[x] > 0) {
        if (decoder->actives) {
          ...
          // XXX this approach is a little sensitive to one of them re-syncing
          // on something
        } else {
          _dispatch(decoder, x);
        }
...
static void _shiftin(fsync_decoder_t *decoder, int x) {
  ...
  if (decoder->synclow[x] & 0x80000000) decoder->synchigh[x] |= 1;

  decoder->synclow[x] <<= 1;
  if (bit) decoder->synclow[x] |= 1;
  ...
  switch (decoder->shstate[x]) {
    case 0:
      ...
      if (_onebits(decoder->synchigh[x] ^ 0xaaaa23eb) < FSYNC_GDTHRESH ||
        ...
        decoder->actives++;
        // note, we do sync on the trailing 32 bits and skip state 1 here to
        // ensure we can catch back-to-back messages with the same decoder phase
...
int fsync_decoder_process_samples(fsync_decoder_t *decoder,
                                  fsync_sample_t *samples, int numSamples) {
  fsync_int_t i, j, k;
...
#if defined(FSYNC_SAMPLE_FORMAT_S8)
    value = (((fsync_float_t)sample)) / 256;
    //		printf("%f\n", value);

#elif defined(FSYNC_SAMPLE_FORMAT_U8)
...
#error "no known sample format set"
  ...
  // XXX callback only -- no longer return 1 if good and have a way to get
  // message. ok?

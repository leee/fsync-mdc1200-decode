...
typedef void (*fsync_decoder_callback_t)(
    int cmd, int subcmd, int from_fleet, int from_unit, int to_fleet,
    int to_unit, int allflag, unsigned char *payload, int payload_len,
    unsigned char *raw_msg, int raw_msg_len, void *context, int is_fsync2,
    int is_2400);
...
/*
 ...
 parameters: fsync_decoder_t *decoder - pointer to the decoder object
             fsync_sample_t *samples - pointer to samples (in format set in
 fsync_types.h) int numSamples - count of the number of samples in buffer
...
/*
 ...
 if this is set, the functions fsync_decoder_get_packet and
 fsync_decoder_get_double_packet will no longer be functional, instead the
 callback function is called immediately when a successful decode happens (from
 within the context of fsync_decoder_process_samples)

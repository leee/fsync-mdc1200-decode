  ...
  for (i = 0; i < 7; i++) {
    for (j = 0; j <= 7; j++) {
      ...
      if (ec >= 3) {
        ...
        if (fixi >= 0) data[fixi] ^= 1 << fixj;  // flip
      }
    }
  }
}
...
static void _procbits(mdc_decoder_t *decoder, int x) {
    ...
    if (decoder->du[x].shstate == 2) {
      ...
      if (!decoder->indouble) {
        ...
        switch (data[0]) {
          ...
          default:
            for (k = 0; k < MDC_ND; k++)
              decoder->du[k].shstate = -1;  // only in the single-packet case,
                                            // double keeps rest going
            break;
        }
      } else {
  ...
  if (decoder->good) {
    if (decoder->callback) {
      (decoder->callback)(
          (int)decoder->good, (unsigned char)decoder->op,
...
static void _shiftin(mdc_decoder_t *decoder, int x) {
  ...
  switch (decoder->du[x].shstate) {
    ...
    case 0:
      ...
      if (decoder->du[x].synclow & 0x80000000) decoder->du[x].synchigh |= 1;
      ...
      if (gcount <= MDC_GDTHRESH) {
        // printf("sync %d  %x %x \n",gcount,decoder->du[x].synchigh,
        // decoder->du[x].synclow);
        decoder->du[x].shstate = 1;
        decoder->du[x].shcount = 0;
        _clearbits(decoder, x);
...
static void _nlproc(mdc_decoder_t *decoder, int x) {
  ...
  switch (decoder->du[x].nlstep) {
    case 3:
      vnow = ((-0.60 * decoder->du[x].nlevel[3]) +
              (.97 * decoder->du[x].nlevel[1]));
      vpast = ((-0.60 * decoder->du[x].nlevel[7]) +
               (.97 * decoder->du[x].nlevel[9]));
      break;
    case 8:
      vnow = ((-0.60 * decoder->du[x].nlevel[8]) +
              (.97 * decoder->du[x].nlevel[6]));
      vpast = ((-0.60 * decoder->du[x].nlevel[2]) +
               (.97 * decoder->du[x].nlevel[4]));
      break;
    default:
      return;
  }
